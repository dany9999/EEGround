...
class CHBMITAllSegmentsLabeledDataset(Dataset):
        ...
        for patient in patient_ids:
            ...
            for fname in sorted(os.listdir(patient_folder)):
                ...
                for i in range(0, n_segments - 1, 2):
                    ...
                    seg_end = seg_start + 2 * self.segment_duration_sec  # 8 secondi
                    label = 0
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(fpath, 'r') as f:
            signals = f['signals']
            x1 = signals[seg_idx][:16]  # primo segmento
            x2 = signals[seg_idx + 1][:16]  # segmento successivo
            x_8s = np.concatenate([x1, x2], axis=-1)  # unione → 8s a 250Hz
