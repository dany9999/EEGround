...
from utils import load_config, EEGDataset, get_all_h5_files_from_tuh
...
def prepare_dataloader_TUH(config):
    base_dir = os.path.abspath(os.path.join("..", "..", "Datasets/TUH"))
    sub_datasets = ['TUAB', 'TUEP', 'TUEV', 'TUSZ']

    all_files = get_all_h5_files_from_tuh(base_dir, sub_datasets)
    all_files.sort()
...
def pretrain(config):
    ...
    train_loader, valid_loader = prepare_dataloader_TUH(config)
