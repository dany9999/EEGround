...
class CHBMITAllSegmentsLabeledDataset(Dataset):
    def __init__(self, patient_ids, data_dir, gt_dir,
                 segment_duration_sec=4, transform=None, mu=None, sigma=None):
        """
        Dataset CHB-MIT:
...
def make_loader(patient_ids, dataset_path, gt_path, config, shuffle=True, balanced=False, neg_to_pos_ratio=5, mu=None, sigma=None):
...
if __name__ == "__main__":
    ...
    mu = np.save("mu_train_finetuning_4s_18channel.npy")
    sigma = np.save("sigma_train_finetuning_4s_18channel.npy")
